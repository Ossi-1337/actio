...
Current status: early implementation. The CLI can run YAML workflows from `.workflows/`, execute `run:` steps in Docker, run local composite, Docker image, public GitHub composite, basic Node 20 JavaScript, and Dockerfile actions, persist run history/logs/artifacts under `ACTIO_HOME`, inspect local action cache entries, and show runs in a localhost web UI.
